...
instance Eq Point where
  Point {coordinates = (a, b)} == Point {coordinates = (x, y)} =
    a == x && b == y
...
instance Mon Vec where
  ...
  (><) Vec {direction = Point {coordinates = (a, b)}} Vec {direction = Point {coordinates = (x, y)}} =
    Vec {direction = Point {coordinates = (a + x, b + y)}}
...
(&) Picture {pictureLines = l1} Picture {pictureLines = l2} =
  Picture {pictureLines = l1 ++ l2}
...
scaleLine i (p1, p2) =
  (roundPoint (scalePoint i p1), roundPoint (scalePoint i p2))

scalePoint :: Integer -> Point -> Point
scalePoint i Point {coordinates = (a, b)} =
  Point {coordinates = ((i % 1) * a, (i % 1) * b)}
...
rotateSine d =
  if d < (180 % 1)
    then bhaskaraSineApproximation d
    else -(bhaskaraSineApproximation (d - 180))

rotateCosine :: R -> R
rotateCosine d = rotateSine (normalizeDegree (d + 90))

applyAuxTransformToPoint :: AuxTransform -> Point -> Point
applyAuxTransformToPoint (VectorTransform translation) p =
  direction (Vec {direction = p} >< translation)
applyAuxTransformToPoint (RotationTransform rotation) Point {coordinates = (r1, r2)} =
  Point {coordinates = (r1 * c - r2 * s, r1 * s + r2 * c)}
  where
    s = rotateSine rotation
    c = rotateCosine rotation

newtype Transform =
  TranformsList [AuxTransform]
  deriving (Eq, Show)
...
normalizeDegree r =
  if r > 0
    then r -
         (quot (numerator r) (denominator r * fullCircleInDegrees) *
          fullCircleInDegrees %
          1)
    else r -
         ((quot (numerator r) (denominator r * fullCircleInDegrees) *
...
transform t Picture {pictureLines = pl} =
  Picture {pictureLines = map (trline t) pl}
